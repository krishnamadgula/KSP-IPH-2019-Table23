...
/* Can also be use to add an input ICON if necessary ' */
// const starPath =
...
class TimePickerWrapper extends React.Component {
  constructor (props) {
    ...
    this.state = {
      open: false,
      useIcon: true,
      value: this.props.defaultValue ? moment(this.props.defaultValue, 'HH:mm:ss') : null
    }
  ...
  onChange (value) {
    ...
    this.props.onChange(value && value.format('HH:mm:ss'))
  }

  render () {
    // const inputIcon = this.getIcon(starPath, iconStyle)
    const { useIcon, open, value } = this.state
    const { disabled, placeholder, showSecond, minuteStep} = this.props
    const clearIcon = this.getIcon(redoPath, { ...iconStyle, right: 0 })
    return (
      <div>
        <TimePicker
          style={{
